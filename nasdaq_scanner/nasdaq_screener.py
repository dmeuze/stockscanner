...
def get_nasdaq_symbols():
    """Geeft alleen de opgegeven lijst van symbols terug."""
    return [
        "PACB", "GEVO", "FCEL", "ACB", "GAME", "GME", "AVTX", "GCT", "PLTR", "BYND",
        "PLUG", "BNGO", "NET", "VRSN", "AMD", "AVGO", "AMZN", "MSFT", "REKR", "NVDA", "GOOGL", "PLUG"
    ]

def run_screener(strategy=None, symbols=None, timeframe='1d'):
    """Main function to run the NASDAQ screener. If strategy is given, only that strategy is run. Als symbols is opgegeven, scan alleen deze. Timeframe is voorbereid voor toekomstige uitbreiding."""
    if symbols is None:
        symbols = get_nasdaq_symbols()

    # yfinance ondersteunt alleen '1d' en hoger, dus we gebruiken altijd '1d' voor nu
    yf_timeframe = '1d'
    if timeframe in ['1d', '1h', '4h', '15m', '5m']:
        # In de toekomst kun je hier mapping toevoegen
        yf_timeframe = '1d'
    ...
    print(f"Start NASDAQ screener... (timeframe: {timeframe})")
    for symbol in symbols:
        print(f"\nAnalyseren van {symbol}...")
        try:
            stock = yf.Ticker(symbol)
            df = stock.history(period="1y")  # yfinance ondersteunt alleen '1d' candles
            if df.empty:
    ...
    filename = f"results/nasdaq_{strategy if strategy else 'all'}_{timestamp}.json"
